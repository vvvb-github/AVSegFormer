...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        qkv = self.in_proj(x).reshape(B, N, 3, self.num_heads,
...
class WindowedAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        qkv = self.in_proj(x).view(-1, N_, 3, self.num_heads,
                                   C // self.num_heads).permute(2, 0, 3, 1, 4)
        # make torchscript happy (cannot use tensor as tuple)
        q, k, v = qkv.unbind(0)
        ...
        x = (attn @ v).transpose(1, 2).reshape(-1,
...
class BertLayer(nn.Module):
    def __init__(self, hidden_size=768, intermediate_size=3072, num_attention_heads=12,
        ...
        if windowed:
            ...
            self.self_attn = Attention(
                hidden_size, num_attention_heads, qkv_bias=True, attn_drop=0., proj_drop=0.)
    ...
    def forward(self, x, H, W):
        ...
        def _inner_forward(x):
            x = x + self.gamma_1 * \
                self.drop_path(self.self_attn(self.norm1(x), H, W))
            x = x + self.gamma_2 * \
                self.drop_path(self.ffn_forward(self.norm2(x)))
            return x
    ...
    def forward(self, x, start_time=0):
        if isinstance(x, int):
            ...
            position_ids = torch.arange(
                x_size, dtype=torch.long, device=x.device) + start_time
...
class TokenBaseEmbedding(nn.Module):
    ...
    def forward(self, input_ids, time_step=None, start_time=0):
        ...
        position_embeddings = self.embeddings_pos(
            pos_inputs, start_time=start_time)
        embeddings = embeddings + position_embeddings.to(embeddings.dtype)

        embeddings_token_type = self.embeddings_token_type.weight[0].unsqueeze(
            0).unsqueeze(1)
...
class UnifiedBertEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
                 ...
                 window_attn=False, window_size=14, with_cp=False, pretrained=None):
